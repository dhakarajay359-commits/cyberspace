...
def create_lobby():
    ...
    conn = sqlite3.connect('users.db')
    c = conn.cursor()
    c.execute("INSERT INTO lobbies (id, host_username, max_players, scenario, red_invite_code, blue_invite_code, custom_desc, custom_flag) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", 
              (lobby_id, session['user'], max_players, scenario, red_invite_code, blue_invite_code, custom_desc, custom_flag))
...
def game_attack():
    ...
    # Store the raw payload for Blue Team investigation
    if len(game.get('real_payloads', [])) > 50: game['real_payloads'].pop(0)
    game.setdefault('real_payloads', []).append({"timestamp": timestamp, "payload": payload, "blocked": blocked})
    ...
    log_entry = f"<span class='text-yellow-500'>[{timestamp}] [WARNING] Anomalous Request Detected. Signature Unknown.</span>"
    game['logs'].append(log_entry)
    if len(game['logs']) > 50: game['logs'].pop(0)
...
def game_defend():
    ...
    return jsonify({"success": True, "rules": game['rules'], "message": f"Defense deployed successfully."})
...
def fetch_youtube_results(query):
    ...
    for item in contents:
        if 'videoRenderer' in item:
            ...
            # Heuristic to filter out official music labels which ALWAYS block embedding
            is_restricted = any(label in owner for label in ['VEVO', 'RECORDS', 'T-SERIES', 'MUSIC', 'SONY'])
            if is_restricted:
                continue
